...
class TransferTestsToCollection extends Command
{
    protected $signature = 'protokit:transfer_tests_to_collection';
    ...
    public function handle(): void
    {
        ...
        $collectionName = $this->ask('Enter collection name', config('app.name'));

        $path = base_path('storage/protokit/tests');
        ...
        Artisan::call('protokit:test', [], new StreamOutput($stream));
        ...
        foreach ($items as $name => $item) {
            if (isset($item['is_request'])) {
                ...
                if ($formdata) {
                    ...
                    $requestBody['raw'] = json_encode($requestBody['raw'], $jsonOptions);
                }
